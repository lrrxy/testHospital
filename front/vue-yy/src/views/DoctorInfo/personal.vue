    ...
    <el-container>
      ...
      <el-aside width="200px">
        <el-menu background-color="#353744" text-color="#fff">
          <el-menu-item index="doctorLayout" :class="{ active: activePath === 'doctorLayout' }" @click="menuClick('doctorLayout')">
            <i class="el-icon-s-home" style="font-size: 18px;"> 首页</i>
          </el-menu-item>
          <el-menu-item index="orderToday" :class="{ active: activePath === 'orderToday' }" @click="menuClick('orderToday')">
            <i class="el-icon-news" style="font-size: 18px;"> 医嘱管理列表</i>
          </el-menu-item>
          <el-menu-item index="doctorOrder" :class="{ active: activePath === 'doctorOrder' }" @click="menuClick('doctorOrder')">
            <i class="el-icon-monitor" style="font-size: 18px;"> 历史挂号列表</i>
          </el-menu-item>
          <el-menu-item index="doctorCard" :class="{ active: activePath === 'userSelect' }" @click="menuClick('userSelect')">
            <i class="el-icon-user-select" style="font-size: 18px;"> 患者管理列表</i>
          </el-menu-item>
          <el-menu-item index="doctorCard" :class="{ active: activePath === 'doctorCard' }" @click="menuClick('doctorCard')">
      ...
      <el-main>
        ...
        <div class="user-info">
          <div class="user-item">
            <label>姓名:</label>
            <el-input v-model="user.doctorName" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>年龄:</label>
            <el-input v-model="user.doctorAge" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>性别:</label>
            <el-input v-model="genderDisplay" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>手机号:</label>
            <el-input v-model="user.doctorPhone" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>身份证号:</label>
          ...
          <div class="user-item">
            <label>邮箱:</label>
            <el-input v-model="user.doctorEmail" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>职位:</label>
            <el-input v-model="user.doctorPosition" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>所属科室:</label>
            <el-input v-model="user.departmentName" disabled class="user-input" />
          </div>
          <div class="user-item">
            <label>评分:</label>
            <el-input v-model="user.doctorScore" disabled class="user-input" />
          </div>
        </div>
      </el-main>
    </el-container>
...
export default {
  ...
  computed: {
    genderDisplay() {
      return this.user.doctorSex === '0' ? '男' : '女';
    }
  },
  mounted() {
  ...
  methods: {
    async fetchDoctorInfo() {
      try {
        ...
        const response = await axios.get('http://localhost:8090/api/doctor/info', {
          headers: { token: token }
        });
        if (response.data.code === 1) {
          this.user = response.data.data; // 更新用户信息
        } else {
