          ...
          <el-table :data="caseData" stripe>
            ...
            <el-table-column label="挂号状态">
              <template #default="scope">
                <el-tag :type="scope.row.caseStatus === 3 ? 'success' : 'danger'">
                  {{ scope.row.caseStatus === 3 ? '已同意' : '未同意' }}
                </el-tag>
              </template>
            </el-table-column>
  ...
  methods: {
    async fetchCaseData() {
      try {
        ...
        const response = await axios.get('http://localhost:8090/api/doctor/selectPay', {
          headers: {
    ...
    requestOrders() {
      this.fetchCaseData();  // 根据患者id查询病例数据
    },
