<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="医院地址" prop="hospitalDizhi">
        <el-input
          v-model="queryParams.hospitalDizhi"
          placeholder="请输入医院地址"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="hospitalList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="医院地址" align="center" prop="hospitalDizhi" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="医院地址" prop="hospitalDizhi">
          <el-input v-model="form.hospitalDizhi" placeholder="请输入医院地址" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加病人所处医院信息";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const userId = row.userId || this.ids
      getHospital(userId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改病人所处医院信息";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除病人所处医院信息编号为"' + userIds + '"的数据项？').then(function() {
        return delHospital(userIds);
      }).then(() => {
        this.getList();
